...
class GoogleAuthenticator(
    ...
    private fun authorize(oAuthLogin: OAuthLogin): GoogleAccessToken =
        runCatching {
            fetchAccessToken(oAuthLogin)
        }.getOrElse { throw AppException(ErrorType.FAILED_AUTH, cause = it) }

    private fun fetchGoogleUser(token: GoogleAccessToken): GoogleUser =
        restClient
            .get()
    ...
    private fun fetchAccessToken(oAuthLogin: OAuthLogin): GoogleAccessToken =
        restClient
            .post()
